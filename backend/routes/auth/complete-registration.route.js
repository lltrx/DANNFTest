...
completeRegistration.post(
  ...
  async (req, resp) => {
    ...
    if (!/^[\u0600-\u06FF\w]+$/.test(nationalIdNumberString)) {
      return resp.status(400).send({
        message:
          "National ID number must contain only digits and Arabic characters",
      });
    }
    ...
    try {
      ...
      user.billStatus = billStatus;
      user.registrationDate = new Date(); // Set the current date as the registration date
